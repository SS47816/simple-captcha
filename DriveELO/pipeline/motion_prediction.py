...
def convert_classification_to_match_results(prediction_result_folder: Path, match_result_folder: Path) -> None:
    ...
    for pkl_file in prediction_result_folder.glob("*.pkl"):
        ...
        # Load the DataFrame
        result_dict = pd.read_pickle(pkl_file) # a dict with scenario_id as keys
        ...
        for key, value in result_dict.items():
            ...
            match_results.append([scenario_id, model_name, miss_rate, 1 - miss_rate])
...
def main():
    ...
    parser.add_argument('--dataset_path',default="/media/shuo/Cappuccino/Waymo/", type=str, help='path to dataset files')
